...
test('clean-slate contract: в новом owner-path запрещены прямые legacy вызовы', () => {
  const executionOwnerSource = read('clean_slate_v2/src/execution_core/executionOwnerPath.js');
  ...
  for (const forbidden of OWNER_PATH_CONTRACT.forbiddenLegacyOwners) {
    assert.equal(executionOwnerSource.includes(forbidden), false, `execution owner содержит forbidden legacy owner: ${forbidden}`);
